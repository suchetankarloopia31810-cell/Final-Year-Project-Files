...
Redesigned with extra-generous spacing to eliminate all text overlap.
"""
import matplotlib
matplotlib.use('Agg')
...
# ── canvas ────────────────────────────────────────────────────────────────────
fig, ax = plt.subplots(figsize=(9.0, 15.0))   # wider + taller canvas
ax.set_xlim(0, 12)
ax.set_ylim(0, 34)
ax.axis('off')
...
W    = 6.2            # box width
H    = 2.55           # box height  (increased to prevent clipping)
GAP  = 0.85           # vertical gap between boxes
TOP  = 31.2           # centre y of first box
STEP = H + GAP        # centre-to-centre spacing

def box(cx, cy, w, h, title, body, fc, ec, body_fs=8.4, title_fs=9.4):
    ax.add_patch(FancyBboxPatch(
        (cx - w/2, cy - h/2), w, h,
        boxstyle="round,pad=0.03,rounding_size=0.10",
        linewidth=1.5, edgecolor=ec, facecolor=fc, zorder=2))
    # title near top of box
    ax.text(cx, cy + h/2 - 0.33, title, ha='center', va='center',
            fontsize=title_fs, fontweight='bold', color=ec, zorder=3,
            clip_on=False)
    # body below the title — use wrap-friendly linespacing
    ax.text(cx, cy - 0.22, body, ha='center', va='center',
            fontsize=body_fs, color=TXT, zorder=3, linespacing=1.30,
            clip_on=False)
...
steps = [
    ...
    ("S2   SUN DRYING",
     ...
     "Colour change green \u2192 tan/brown indicates moisture loss."),
    ...
    ("S4   GRINDING",
     ...
     "(Dept. of Chemistry, NIT Srinagar) to reduce particle size."),
    ...
    ("S8   COLD PRESSING",
     ...
     "the mixture, reduce porosity and improve inter-particle bonding."),
...
# S7 is index 5
y_mix = ys[5]
SX, SW, SH = 9.8, 3.7, 3.0   # wider/taller side box
box(SX, y_mix, SW, SH,
    "S6   BINDER PREPARATION\n(Parallel Step)",
    "Food-grade corn starch\ngelatinised on a hot plate with\n"
    "magnetic stirrer at 80\u201390 \u00b0C\nuntil a gel forms.\n"
    "Natural binder for S3 (90:10) & S4 (70:30).",
    SIDEF, SIDE, body_fs=7.8, title_fs=8.6)
...
# ── titles & footer ────────────────────────────────────────────────────────────
ax.text(6, 33.0, "PROCESS FLOWSHEET", ha='center',
...
out_path = '/projects/sandbox/Final-Year-Project-Files/Generated_Report/charts/Fig06_Process_Flowsheet.png'
